...
describe('ByteBrigade', function () {
  const ByteBrigade = require('./ByteBrigade')
  it('should allow list of arguments', function () {
    let bb
    bb = new ByteBrigade(4, 4)
    bb.set(0, 15)
    bb.set(1, 15)
    bb.get(0).should.equal(15)
    bb.get(1).should.equal(15)
    bb = new ByteBrigade(3, 5)
    bb.set(0, 5)
    bb.set(1, 13)
...
describe('TimeData', function () {
  ...
  it('should be OK with performance of encoding/decoding', function () {
    ...
    console.log(allData.length / maxcount)
    diffs.push(Date.now() - ts)
    ts = Date.now()
